...
%PRBS
n = (0:N-1);
r2 = zeros(1,N);         % Pre-allocate
r2(1) = (randi([0 1])-0.5)*4;
p = 0.5;
minclock = 2;
clock = 0;
for i = 2:N
...
% Create new input/output data
[u2,y2] = assignment_sys_25(r2);
...
%     OeSys = oe(data,[nb nf nk]);
    ...
    % Save chosen model (6) for comparison later
%     OeSysBest = oe(data,[8 8 0]);
    BjSysBest = bj(data,[5 1 2 5 0]);
    %
    ...
    % Determine fit error
    Nfit = length(E.OutputData)-1;
    Cost(i,1) = i;
    Cost(i,2) = (1/Nfit)*sum(E.OutputData(2:end).^2);
    
    Nfit2 = length(E2.OutputData)-1;
    Cost2(i,1) = i;
    Cost2(i,2) = (1/Nfit2)*sum(E2.OutputData(2:end).^2);
    %
    ...
    % Save cost
    if (i == 1)
        OeMSE = [i, BjSys.Report.Fit.MSE];
    else
        OeMSE = [OeMSE; i,BjSys.Report.Fit.MSE];
    end
end
...
hold on
plot(Cost(:,1),Cost(:,2))
plot(Cost2(:,1),Cost2(:,2))
hold off
grid on
title('Cost over models')
xlabel('Model number [n]')
ylabel('Cost')
legend('Cost','Cost new data')

%% Run simulation for new input/output data for validation
% Create new PRBS input signal
%PRBS
n = (0:N-1);
r = zeros(1,N);         % Pre-allocate
r(1) = (randi([0 1])-0.5)*4;
...
% Compare
% [y,fit,x] = compare(data2,OeSysBest)
[y,fit,x] = compare(data2,BjSysBest)
%
